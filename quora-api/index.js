...
quora.knowsAbout = username => {
	...
	return got(url)
		.then(res => {
			...
			return {
				knowsAbout1: $(
					".TopicPreviewBioAnswers .topic_name, .TopicPreviewItemSentence .topic_name"
				)
					.eq(3)
					.text(),
				answers1: remAns(
					$(".UserAnswersInTopicLink")
						.eq(0)
						.text()
				),
				endorsement1: remEndorse(
					$(".UserTopicEndorsementsLink")
						.eq(0)
						.text()
				),
				knowsAbout2: $(
...
quora.answer = userAnswer => {
	...
	return got(url)
		.then(res => {
			...
			for (let i = 0; i < 200; i++) {
				const cand = $(".ui_qtext_rendered_qtext")
					.eq(i)
					.text();
...
quora.activity = username => {
	...
	return got(url)
		.then(res => {
			...
			return {
				question0:
					$(".feed_item .question_link, .feed_item .BoardItemTitle")
						.eq(0)
						.text() || null,
				action0:
					$(".EventHeader, .ContentReason, .inline_suggestions_reason")
						.eq(0)
						.text() || null,
				question1:
					$(".feed_item .question_link, .feed_item .BoardItemTitle")
			...
			return {
				question0:
					$(".feed_item .question_link, .feed_item .BoardItemTitle")
						...
						.text() || null,
				details0:
					$(".CredibilitySection, .CredibilityFacts")
						.eq(0)
						.text() || null,
				question1:
					$(".feed_item .question_link, .feed_item .BoardItemTitle")
